...
export default function TeacherManagement() {
  ...
  const handleDelete = async (teacherId: string) => {
    if (!isAdmin) {
      showMessage('error', 'No tiene permisos para eliminar docentes');
      return;
    }
  ...
  const handleEdit = (teacher: TeacherWithSubjects) => {
    if (!isAdmin && user?.id !== teacher.user_id) {
      showMessage('error', 'Solo puede editar su propia información');
      return;
    }
  ...
  const handleAdd = () => {
    if (!isAdmin) {
      showMessage('error', 'No tiene permisos para agregar docentes');
      return;
    }
